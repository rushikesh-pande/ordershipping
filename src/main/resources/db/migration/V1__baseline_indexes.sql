...
-- CREATE INDEX IF NOT EXISTS idx_ordershipping_status ON ordershippings (status);

-- Created date (pagination/sorting by date)
-- CREATE INDEX IF NOT EXISTS idx_ordershipping_created_at ON ordershippings (created_at DESC);

-- Composite: status + created_at (covers most list queries)
-- CREATE INDEX IF NOT EXISTS idx_ordershipping_status_created
--     ON ordershippings (status, created_at DESC);

-- Foreign key index (prevents full-table scan on joins)
-- CREATE INDEX IF NOT EXISTS idx_ordershipping_user_id ON ordershippings (user_id);
