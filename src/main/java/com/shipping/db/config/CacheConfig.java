package com.shipping.db.config;
 ...
 *
 * Usage in service:
 *   @Cacheable(value = "ordershippingCache", key = "#id")
 *   @CacheEvict(value = "ordershippingCache", key = "#id")
 *   @CachePut(value  = "ordershippingCache", key = "#result.id")
 */
@Configuration
@EnableCaching
public class CacheConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory factory) {
        ...
        // ── ordershipping specific caches ─────────────────────────────────────
        caches.put("ordershippingCache",       defaultConfig().entryTtl(MEDIUM));
        caches.put("ordershippingListCache",   defaultConfig().entryTtl(SHORT));
        caches.put("ordershippingCountCache",  defaultConfig().entryTtl(SHORT));
        caches.put("ordershippingRefCache",    defaultConfig().entryTtl(LONG));
